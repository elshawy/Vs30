...
reference_coordinates = reference_df[['Station', 'Latitude', 'Longitude', 'Vs30']].values
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.csv') and file_name != reference_file:
        ...
        if 'Latitude' in df.columns and 'Longitude' in df.columns and 'Vs30' in df.columns:
            coordinates = df[['Station', 'Latitude', 'Longitude', 'Vs30']].values
...
for i, ref_coord in enumerate(reference_coordinates):
    for file_name, coord in all_coordinates:
        if is_within_distance((ref_coord[1], ref_coord[2]), (coord[1], coord[2]), distance_threshold):
...
output_file = 'DUP_locations.csv'
with open(output_file, 'w') as f:
    f.write('File1,Station1,Latitude1,Longitude1,Vs30_1,File2,Station2,Latitude2,Longitude2,Vs30_2\n')
    for result in results:
        file1, coord1, file2, coord2 = result
        if coord1[3] == coord2[3]:  # Check if Vs30_1 and Vs30_2 are the same (for inclusion)
            f.write(f"{file1},{coord1[0]},{coord1[1]},{coord1[2]},{coord1[3]},{file2},{coord2[0]},{coord2[1]},{coord2[2]},{coord2[3]}\n")
