...
def _new_mean(mu_0, n0, var, y):
    log_mu_0 = log(mu_0)
    log_y = log(y)
    mean = exp((n0 / var * log_mu_0 + log_y / var) / (n0 / var + 1 / var))
    print(f"Calculating new mean:")
    print(f"log(mu_0): {log_mu_0}, log(y): {log_y}")
    print(f"New mean: {mean}")
    return mean

def _new_var(sigma_0, n0, sigma_y):
    var = 1 / ((n0 / sigma_0 ** 2) + (1 / sigma_y ** 2))
    print(f"Calculating new variance:")
...
def posterior(model, sites, idcol, n_prior=3, min_sigma=0.5):
    ...
    for m, group in groups:
        ...
        print(f"Index Group {m + 1}:")

        for _, row in group.iterrows():
            print(f"Initial vs30: {vs30[m]}")
            print(f"Initial stdv: {stdv[m]}")
            print(f"Initial n0: {n0[m]}")

            var = _new_var(stdv[m], n0[m], row['uncertainty'])
            print(f"Calculated variance: {var}")

            try:
                vs30[m] = _new_mean(vs30[m], n0[m], var, row['vs30'])
            except ValueError as e:
                print(f"Error in group {m + 1}: {e}")
                vs30[m] = np.nan

            stdv[m] = sqrt(var)
            n0[m] += 1

            print(f"Updated vs30: {vs30[m]}")
            print(f"Updated stdv: {stdv[m]}")
            print(f"Updated n0: {n0[m]}")
            print("-----")
