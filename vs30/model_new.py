...
def posterior(model, sites, idcol, n_prior=3, min_sigma=0.5):
    ...
    for m in range(len(model)):
        if m == ID_NODATA or m == 0:
            continue
        group = sites[sites[idcol] == m + 1].copy()
        if group.empty:
            print(f"[DEBUG] Skipping empty group: {m + 1}")
            continue
        print(f"[DEBUG] Start processing group {m + 1}, Data count: {len(group)}")
        group.loc[:, 'W'] = group['q'].apply(lambda x: 0.1 if x == 5 else 1)
        print(f"[DEBUG] Group weights (W): {group['W'].values}")

        # var와 mean 계산
        n = len(group)
        var = _new_var(stdv[m], n0[m], group['W'], group['uncertainty'], n, m)
        print(f"[DEBUG] Calculated var for group {m + 1}: {var}")
        vs30[m] = _new_mean(vs30[m], n0[m], var, group['W'], group['vs30'], n, m)
        print(f"[DEBUG] Updated mean for group {m + 1}: {vs30[m]}")
