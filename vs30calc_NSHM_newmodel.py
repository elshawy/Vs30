...
sites = sites_load_NSHM2022_0.load_vs(source=p_sites.source)
...
for model_setup in [p_geol, p_terr]:
    ...
    if model_setup.update == "prior":
        ...
        if p_sites.source == "cpt":
            ...
            model_table = model_new.cluster_update(model_table, sites, model_setup.letter)
        else:
            model_table = model_new.posterior(model_table, sites, f"{model_setup.letter}id")
...
if p_geol.update != "off" and p_terr.update != "off":
    ...
    if p_ll is not None:
        for prefix in ["", "mvn_"]:
            table[f"{prefix}vs30"], table[f"{prefix}stdv"] = model_new.combine_models(
                p_comb,
        ...
        model_new.combine_tiff(p_paths.out, "combined.tif", p_grid, p_comb, *tiffs)
        model_new.combine_tiff(p_paths.out, "combined_mvn.tif", p_grid, p_comb, *tiffs_mvn)
    print(f"{time()-t:.2f}s")
pool.close()
pool.join()
...
if p_ll is not None:
    ...
    copyfile(
        os.path.join(sites_load_NSHM2022_0.data, "qgis_points.qgz"),
        os.path.join(p_paths.out, "qgis.qgz"),
    )
else:
    copyfile(
        os.path.join(sites_load_NSHM2022_0.data, "qgis_rasters.qgz"),
        os.path.join(p_paths.out, "qgis.qgz"),
    )
