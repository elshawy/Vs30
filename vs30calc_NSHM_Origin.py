...
sites = sites_load_NSHM2022_.load_vs(source=p_sites.source)
...
if p_geol.update != "off" and p_terr.update != "off":
    ...
    if p_ll is not None:
        ...
        model_Origin.combine_tiff(p_paths.out, "combined.tif", p_grid, p_comb, *tiffs)
        model_Origin.combine_tiff(p_paths.out, "combined_mvn.tif", p_grid, p_comb, *tiffs_mvn)
    print(f"{time()-t:.2f}s")
pool.close()
pool.join()
...
if p_ll is not None:
    ...
    copyfile(
        os.path.join(sites_load_NSHM2022_.data, "qgis_points.qgz"),
        os.path.join(p_paths.out, "qgis.qgz"),
    )
else:
    copyfile(
        os.path.join(sites_load_NSHM2022_.data, "qgis_rasters.qgz"),
        os.path.join(p_paths.out, "qgis.qgz"),
    )
