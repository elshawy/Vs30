...
def generate_spatially_correlated_field(depths, num_realizations, correlation_length, mean=0):
    """
    ...
    # Compute std_dev based on depth
    std_dev = np.zeros_like(depths)
    std_dev[depths <= 5] = 0.162
    std_dev[(depths > 5) & (depths < 10)] = 0.216 - 0.0108 * depths[(depths > 5) & (depths < 10)]
    std_dev[depths >= 10] = 0.108

    # Apply correlation via Cholesky decomposition
    correlated_fields = mean + (uncorrelated_random_fields @ L.T) * std_dev
...
# Load the 'Depth' and 'Mc15 Vs' columns from the CSV file
filename = 'SCPT_14541.csv'
data = pd.read_csv(filename, usecols=['Depth', 'Mc15 Vs'])
...
# Multiply the values of the second column by the exponential of the random fields for other columns excepting depths column
for i in range(1, random_fields.shape[0]):
    merged_data[f'Mc15 Vs_{i}'] = merged_data['Mc15 Vs'] * np.exp(merged_data[f'Random_Field_{i}'])
...
data_origin = pd.read_csv('SCPT_14541.csv', usecols=['Depth', 'Mc15 Vs'])
...
plt.plot(data_origin['Mc15 Vs'], data_origin['Depth'], color='red', label='Original Data')
plt.xlabel('Mc15 Vs')
plt.ylabel('Depth (m)')
